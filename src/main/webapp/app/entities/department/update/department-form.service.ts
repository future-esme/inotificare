...
type DepartmentFormDefaults = Pick<NewDepartment, 'id' | 'members'>;

type DepartmentFormGroupContent = {
  id: FormControl<IDepartment['id'] | NewDepartment['id']>;
  title: FormControl<IDepartment['title']>;
  departmentAdmin: FormControl<IDepartment['departmentAdmin']>;
  members: FormControl<IDepartment['members']>;
};
...
export class DepartmentFormService {
  createDepartmentFormGroup(department: DepartmentFormGroupInput = { id: null }): DepartmentFormGroup {
    ...
    return new FormGroup<DepartmentFormGroupContent>({
      ...
      title: new FormControl(departmentRawValue.title, {
        validators: [Validators.required],
      }),
      departmentAdmin: new FormControl(departmentRawValue.departmentAdmin),
      members: new FormControl(departmentRawValue.members ?? []),
    });
  }
